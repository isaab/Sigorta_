...
$('.form-arror').hide();
var faults = $('.selector_form input[required="required"]');
faults.on('focus', function(e) {
    if ($(this).val() == '') {

        var errorTag = $(this).parents('form').parent().find('.form-arror')
...
$('.selector_form').on('submit',function(e) {
    ...
    function result(class_key, data) {
        setTimeout(function() {
            ...
            $('.form-arror').hide();
           $(form).parent().find( '.ajax-result ' + class_key).show().html(data);
          $( '.success').hide(2000);
          $( '.error').hide(2000);
...
$(function() {
    ...
    if ($(".typed-element").length) {
        ...
        $(".typed-element").typed({
            strings: ["Yeni sığorta proqramlarını kəşf edin"],
            typeSpeed: 50,
            backDelay: 3000,
            loop: true
        });
    ...
    // reviews_carousel
    if ($('.js-clients_twitter').length) {

        $('.js-clients_twitter').slick({
            slidesToShow: 1,
            arrows: true,
            dots: true,
    ...
    $('.tabs-header').on('click', 'li', function(e) {
        ...
        if (elem === '') {
            ...
            $('.pop_up_tabs').find('.tabs-item').hide()
            $('' + elem).fadeIn(200)
